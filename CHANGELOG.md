...
### 新增
- 🆕 **Git Revert 功能 - 快速撤销修改**
  - **资源管理器右键菜单**：右键文件/文件夹选择"撤销对文件的修改"/"撤销对文件夹的修改"
  - **终端右键菜单**：在终端选中文件路径，右键选择"撤销文件修改"
  - **确认对话框**：执行前显示确认对话框，防止误操作
  - **批量撤销**：支持从终端选中多个文件路径批量撤销
  - **智能检测**：自动检测文件是否有修改，无修改时提示无需撤销
  - **Git 版本兼容**：优先使用 `git restore` (Git 2.23+)，自动回退到 `git checkout HEAD --`
  - **完整反馈**：显示操作进度、成功/失败提示、详细日志记录
  - **中英文支持**：所有提示消息和确认对话框完全国际化
